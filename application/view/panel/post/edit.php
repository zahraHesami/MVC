    ...
    &&  isset($_POST['body']) && $_POST['body'] !== '')
{
    ...
    if(isset($_FILES['image']) && $_FILES['image']['name'] !== '')
    {
        ...
        {
            $query = 'UPDATE  php_project.posts SET title = ?, cat_id = ?, body = ?, image = ?, update_at = NOW() WHERE id = ? ;';
            $statement = $pdo->prepare($query);
        ...
        if($category !== false)
        {
            $query = 'UPDATE  php_project.posts SET title = ?, cat_id = ?, body = ?, update_at = NOW() WHERE id = ? ;';
            $statement = $pdo->prepare($query);
                ...
                <form action="<?= url('panel/post/edit.php?post_id=' . $_GET['post_id']) ?>" method="post" enctype="multipart/form-data">
                    ...
                    <section class="form-group">
                        ...
                        <select class="form-control" name="cat_id" id="cat_id">
                            <?php
                            global $pdo;
                            $query = "SELECT * FROM php_project.categorizes";
                            $statement = $pdo->prepare($query);
                            $statement->execute();
                            $categories = $statement->fetchAll();
                            foreach ($categories as $category) { ?>
                                <option value="<?= $category->id ?>" <?php if($category->id == $post->cat_id) echo 'selected'; ?>> <?= $category->name_cat ?> </option>
                            <?php } ?>
                        </select>
                    </section>
                    <section class="form-group">
